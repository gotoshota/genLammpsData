...
def write_lammps_data(chains, box_size, filename="lmp.data"):
    nchains = len(chains)
    nbeads = len(chains[0])
    with open(filename, "w") as f:
        # ヘッダー情報の書き込み
        total_atoms = len(chains) * len(chains[0])
        ...
        for chain_id, chain in enumerate(chains, start=1):
            for bead in chain:
                ...
                f.write(f"{atom_id} {chain_id} 1 {position[0]:.3f} {position[1]:.3f} {position[2]:.3f} {nx} {ny} {nz}\n")
                atom_id += 1
...
def main():
    ...
    write_lammps_data(chains, box_size, args.output)
